...
export default function CasesClient() {
  ...
  const slaRisk = getParam("slaRisk") === "1";
  const severity = getParam("severity") as "sev1" | "sev2" | "sev3" | undefined;
  const status = getParam("status") as "open" | "investigating" | "waiting" | "resolved" | undefined;
  const ownerTeam = getParam("owner") as "NOC" | "Service Delivery" | "Provider" | undefined;
  ...
  function onSearchSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    router.replace(buildHref("/cases", baseParams, { q: trimmed ? trimmed : undefined }));
  }
    ...
    <>
      <div className="cx-panel rounded-2xl p-4 md:p-5 mb-6">
        ...
        <div className="mt-4 grid lg:grid-cols-2 gap-4">
          ...
          <div>
            <div className="mt-4 text-xs uppercase tracking-wide cx-muted">Status</div>
            <div className="mt-2 flex flex-wrap gap-2">
              {statusOptions.map((s) => {
