...
export default function WorkClient() {
  ...
  const status = getParam("status") as "blocked" | "in_progress" | "done" | "planned" | undefined;
  const blockedReason = getParam("blockedReason");
  const serviceType = getParam("serviceType") as "internet" | "sd-wan" | "security" | undefined;
  const blockedOwnerTeam = getParam("blockedOwnerTeam") as "Service Delivery" | "Provider" | "Customer" | "NOC" | undefined;
  const atRisk = getParam("atRisk") === "1";
  ...
  function onSearchSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    router.replace(buildHref("/work", baseParams, { q: trimmed ? trimmed : undefined }));
  }
    ...
    <>
      <div className="cx-panel rounded-2xl p-4 md:p-5 mb-6">
        ...
        <div className="mt-4 grid lg:grid-cols-2 gap-4">
          ...
          <div>
            <div className="mt-4 text-xs uppercase tracking-wide cx-muted">Queue</div>
            <div className="mt-2 flex flex-wrap gap-2">
              {statusOptions.map((s) => {
        ...
        <div className="mt-4 grid lg:grid-cols-2 gap-4">
          ...
          <div>
            <div className="mt-4 text-xs uppercase tracking-wide cx-muted">Blocked owner</div>
            <div className="mt-2 flex flex-wrap gap-2">
              {ownerOptions.map((o) => {
